classdef LennardJonesPotential < handle
    ...
    methods (Access = public)
        ...
        function [] = removeMolecule(obj, idx)
            obj.x(idx) = [];
            obj.y(idx) = [];
            obj.v(idx) = [];
            obj.u(idx) = [];
            ...
            % make sure everything still column vectors (empty matrixes
            % seem to revert to row vectors??)
            if obj.N == 0
                obj.x = obj.x';
                obj.y = obj.y';
                obj.v = obj.v';
        ...
        function [] = addMolecule(obj, x, y, v, u, m)
            obj.x(end+1,1) = x;
            obj.y(end+1,1) = y;
            obj.v(end+1,1) = v;
            obj.u(end+1,1) = u;
